...
vec <- c(a = "hello", b = 1)
```
**1 is automatically transformed to a string"**
...
```{r}
mode <- function(df, var) {
  df %>% group_by({{var}}) %>%  count(sort=TRUE) %>% .[1,1]
...
# if neutral, then :|
# 
f_emotions <- function(df, var) { 
  
  if (df %>% filter({{var}}=="netural") %>% nrow() >0) {
    df %>% mutate({{var}}:=case_when({{var}}=="happy" ~ ":)", 
                                     {{var}}=="sad" ~ ":(",
                                     {{var}}=="neutral" ~ ":|",
                                     {{var}}=="netural" ~ ":|"))
  } else {
...
f_emotions(study, emotions)
#note that the if statement is not really needed. One could somply add
  #a line in case_when with netural
study %>% filter(emotions=="netural") %>% nrow() >0
```
...
```{r}
mean_sd_num <- function(dat) {
  if(is.numeric(dat)) {
    mean <- sum(dat, na.rm=TRUE)/length(dat)
...
```{r}
map(study, mean_sd_num)
#or with map_if
map_if(study, is.numeric, \(x) {
  c(sum(x, na.rm=TRUE)/length(x),
    sd(x, na.rm=TRUE))
})
```
...
```{r}
map_if(study, is.character, tolower) %>% as.data.frame()
```
...
#install.packages("LegislatoR") added
...
#legislatoR::cld_content() "ger" replaced with "deu"
#"session" replaced by session
...
# sessions served
sessions_served_df <- 
  get_political(legislature = "deu") %>%  #%>% had to be edited
  group_by(pageid) |> 
...
get_age <- function(birth, date_at) {
  ...
  #difftime calculated first minus second argument such that arguments had to be switched
...
#dataframe was missing in select statement
...
ggplot(legislator_df_table, aes(y = Mean, x = -Rank)) + 
  ...
  geom_col(stats = "identity") + # change to geom_col
...
fmla <- paste("traffic_log", paste(covars, collapse = " + "), sep = " ~ ") #minus had to be replaced by +
...
usethis::create_from_github(
  "https://github.com/intro-to-data-science-23/labs.git",
  destdir = "C:/Users/sarah.wetzel/Documents/HIGH/Vorlesungen, Seminare und Vorträge/DataScienceWithR/Labs",
  fork = TRUE
)
